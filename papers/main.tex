...
\centering
\footnotesize
\caption{Greedy test accuracy for each agent across dataset, taken from the end-of-training final eval (\texttt{summary.csv} where available, else last row of \texttt{learning\_curves.csv}). All ours runs use the dynamic action space; horizon $10^7$ steps. ConPoLe / ConPoLe-local numbers from~\cite{poesia2021contrastive}. Both \textsc{ppo-tree-rc} and \textsc{ppo-tree-rc-buf} exceed the ConPoLe SOTA on poesia.}
...
\midrule
ppo               & 0.04  & 0.02  & 0.17  \\
ppo-tree          & 0.24  & 0.01  & 0.74  \\
ppo-tree-rc       & \textbf{0.90} & 0.01  & \textbf{0.96} \\
ppo-tree-buf      & 0.47  & --    & 0.87  \\
ppo-tree-rc-buf   & 0.80  & \textbf{0.44} & 0.93 \\
\bottomrule
