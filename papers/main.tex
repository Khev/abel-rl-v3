...
\subsection{Full method stack: 0.62 with no per-evaluation tuning}
\label{sec:full_stack}
Putting it together -- TreeMLP-PPO with relabel-constants, success-replay, CoV macroaction, anti-loop penalty $\alpha = 0.1$, and $\texttt{custom\_cbrt}$, then plain beam search with canonical-state dedup -- one fresh seed (referred to as \texttt{seed10000}) reaches \textit{test}$_{\text{beam}} = \mathbf{0.62}$ on \texttt{mixed\_v2\_easy} after 3M training steps. This is more than $2\times$ the plain-beam baseline ($0.27$) at the same step budget. Greedy on this same checkpoint reaches $0.31$; beam search at $\text{max\_steps} = 10$ with dedup accounts for the remaining lift. Notably, value-guided beam ($\lambda \geq 0.5$) \emph{hurts} this checkpoint by $1$--$3$ percentage points, mirroring the seed9100 anti-loop observation: when the trained policy is already sharp, the value head signal becomes noise. We have only $n=1$ at the full-stack configuration in this paper; multi-seed reproduction is the obvious next step.

\subsection{Per-class analysis: what each method actually unlocks}
\label{sec:per_class}
Aggregate \textit{test}$_{\text{beam}}$ numbers hide a striking class-dependent structure. Table~\ref{tab:per_class} breaks down accuracy by the seven source classes of \texttt{mixed\_v2\_easy} for our three representative checkpoints: a baseline seed (8001), an unlucky baseline seed (7001), and our anti-loop seed (9100).
...
\begin{tabular}{lccc}
\toprule
class (n)            & seed7001 (base)    & seed8001 (base)    & seed9100 (anti-loop) \\
\midrule
quartic (15)         & 0/0.20/\textbf{1.00} & 0/\textbf{1.00}/1.00 & 0/\textbf{0.00}/0.00 \\
quadratic (15)       & 0/0.00/0.00        & 0.80/0.93/0.93     & 0.87/0.93/0.87       \\
cubic (15)           & \textbf{0/0/0}     & 0/0/0              & 0/0/0                \\
exponential (15)     & \textbf{0/0/0}     & 0/0/0              & 0/0/0                \\
abel\_level3 (20)    & 0/0.20/0.20        & 0.05/0.15/0.25     & 0.25/\textbf{0.40}/0.40 \\
abel\_level2 (10)    & 0.10/0.30/0.30     & 0.10/0.50/0.50     & 0.10/\textbf{0.70}/0.70 \\
abel\_level1 (1)     & 1/1/1              & 0/1/1              & 1/1/1                \\
\bottomrule
\end{tabular}
\caption{\textbf{Per-class test accuracy} on \texttt{mixed\_v2\_easy}, shown as greedy / plain-beam / value-beam fractions. Notable cells in bold. Each method unlocks a different class.}
\label{tab:per_class}
\end{table}

Five findings emerge.

\textbf{(i) Cubic and exponential are unsolved across the board.} 30 of the 91 test equations ($33\%$) lie in classes where every checkpoint scores zero on every metric. Inspection reveals the cause is action-set limited rather than training-data limited: after the depressed-cubic CoV $x \to x - b/(3a)$ the equation collapses to $a y^3 + d = 0$, which requires a cube-root operation -- but our action set contains only \texttt{square}, \texttt{sqrt}, \texttt{log}, \texttt{exp}, \texttt{sin}, $\sin^{-1}$, \texttt{cos}, $\cos^{-1}$, and multiplication by $-1$. There is no \texttt{cbrt}. Exponential equations are in principle solvable (CoV $x \to \log y / k$ yields a quadratic in $y$), but no checkpoint has discovered the recipe -- suggesting they are training-data limited and would benefit from oversampling that class.

\textbf{(ii) The quartic class is fragile.} Baseline seed8001 solves all 15 quartics with plain beam -- the ``depressed-quartic 6-step recipe'' (\texttt{COV REL - / SRT SRT}) from Section~\ref{sec:antiloop}. Baseline seed7001 has the recipe but cannot find it under greedy or plain beam (3/15); value-guided beam unlocks it (15/15) because the policy ranks the correct first action highly but not top-1. Anti-loop seed9100 has \emph{lost} the quartic class entirely (0/15): the action-diversity penalty broke the recipe, which has \texttt{relabel\_const} as action 2 and is naturally repetitive.

\textbf{(iii) Anti-loop trades quartic memorization for closed-equation breadth.} While losing quartic, seed9100 gains on \texttt{abel\_level3} ($0.40$ vs.\ $0.15$-$0.25$) and \texttt{abel\_level2} ($0.70$ vs.\ $0.30$-$0.50$). The penalty pushes the policy away from any single repetitive recipe, producing a more general but less peaked solver. This is the qualitative shift behind the aggregate gain in Section~\ref{sec:antiloop}.

\textbf{(iv) Value-guided beam specifically helps when the policy is unsharp.} The seed7001 quartic case ($0.20 \to 1.00$) is the cleanest example: the value head correctly identifies the productive path that the policy ranks below top-1. On checkpoints where the policy already places the right action first (seed8001 quartic, seed9100 closed eqns), value-beam gives no further lift.

\textbf{(v) Methods are complementary by class.} No single method dominates. Best-of-three by class gives a hypothetical \textit{test}$_{\text{beam}}$ of $\approx 0.60$ -- substantially higher than any single method. Combining methods (e.g., training a model with anti-loop \emph{and} a separate quartic-specialist model, then voting) is a natural extension.

The cubic action-set limitation and the exponential training-data gap are the two largest sources of remaining test-set error. Both are concrete, addressable shortcomings rather than fundamental limits of the approach.
