...
\textbf{Curiosity based rewards}. In previous work~\cite{dabelow2024symbolic}, curiosity-based exploration bonuses (e.g.\ ICM, RND, NGU) were shown to enhance learning on a vanilla MLP policy. Stacking the same bonuses on top of our TreeMLP, however, \emph{degrades} performance (Figure~\ref{curiosity_hurts} in the appendix). We therefore do \textit{not} use curiosity in our headline results.
...
\centering
\footnotesize
\caption{Greedy test accuracy for each agent across dataset. Numbers for the four \textsc{ppo-tree}* variants are end-of-training values ($10^7$ steps) read from Figure~\ref{learning_curves_closed}. The plain \textsc{ppo} (MLP) row is from a separate run with the same dynamic action space. ConPoLe / ConPoLe-local numbers from~\cite{poesia2021contrastive}. Both \textsc{ppo-tree-rc} and \textsc{ppo-tree-rc-buf} match or exceed the ConPoLe SOTA on poesia.}
...
ConPoLe           & --    & --    & 0.925 \\
ppo               & 0.04  & 0.02  & 0.17  \\
ppo-tree          & 0.35  & 0.10  & 0.80  \\
ppo-tree-rc       & 0.85  & 0.10  & 0.83  \\
ppo-tree-buf      & 0.65  & 0.38  & 0.85  \\
ppo-tree-rc-buf   & 0.85  & 0.50  & \textbf{0.93} \\
\bottomrule
...
\begin{itemize}
    ...
    \item Adding curiosity bonuses (ICM, RND, NGU) on top of TreeMLP \emph{degrades} learning on \texttt{abel\_level3} (Figure~\ref{curiosity_hurts}): at $5{\times}10^5$ steps every curiosity variant trails the no-curiosity baseline on coverage and all three accuracy metrics, with RND failing to make any meaningful progress in $500$k steps. This contrasts with earlier work~\cite{dabelow2024symbolic}, where curiosity \emph{improved} a vanilla MLP policy. A plausible explanation: the TreeMLP's structural inductive bias already supplies the exploration signal that curiosity provides for the MLP, making the two redundant or actively conflicting.
\end{itemize}
...
\begin{figure}[t]
  ...
  \includegraphics[width=\linewidth]{curiosity-tree-hurts.png}
    \caption{Curiosity bonuses (ICM, NGU, RND) degrade TreeMLP learning on \texttt{abel\_level3}. All four panels (coverage, greedy, beam, success@10) at the same training budget ($5{\times}10^5$ steps) show every curiosity variant tracking below the no-curiosity baseline. RND in particular fails to make meaningful progress. Baseline is mean $\pm$ min/max across $n{=}5$ seeds; curiosity variants are single seed.}
