...
\end{align}
where the rhs of each shows a solving CoV (the polynomial families are restricted to fully-depressable special forms; see Appendix). The key design choice is \emph{how} to handle the CoV. We do not learn the substitution generatively. Instead we expose the CoV as a single \emph{macroaction} within the same dynamic action space as the closed-equation manipulations: when the policy selects the CoV macroaction, the substitution $\phi$ is computed analytically (the pattern-match is exact for the four families above) and applied to the current equation. The agent therefore learns only \emph{when} to invoke a CoV and how to compose it with ordinary algebraic steps -- not the substitution itself. This keeps the open-equation problem inside the same MDP and policy as the closed case, and a single policy is trained on a mixed dataset of closed and open equations (Section~\ref{sec:open_method}).

\subsection{Method}
\label{sec:open_method}
We train a single TreeMLP-PPO policy on mixed datasets of closed and open equations: \texttt{mixed\_v2\_easy} ($916$ train / $91$ test, stratified across the four CoV families and the \texttt{abel\_level1/2/3} closed classes) and a $10\times$-larger \texttt{mixed\_v2\_large}. The policy carries over the closed-equation inductive biases -- curriculum learning, constant relabeling, and a success-replay buffer (Section~\ref{sec:closed}) -- with four additions specific to the open setting, which we state up front and ablate below.
...
A plain \textsc{ppo-tree-rc-buf-cov} agent on \texttt{mixed\_v2\_easy} reaches $\textit{coverage}=1.0$ on the training set but only $\textit{test}_{\text{beam}} \approx 0.27$: it memorizes one ``depressed-quartic'' recipe and fails to generalize. Adding the four ingredients above, selecting the best checkpoint by early-stopping, and decoding with value-guided beam ($\lambda = 1$) raises this to $\textit{test}_{\text{beam}} = \mathbf{0.80}$ ($73/91$ test equations) -- nearly $3\times$ the baseline. Figure~\ref{fig:open_curves} shows the learning curves.

\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{../figures/open_eqn_curves.png}
    \caption{Open-equation learning curves on \texttt{mixed\_v2\_easy} ($91$ test equations): training-set coverage, greedy test accuracy, and beam-search test accuracy versus training steps, for the baseline, the baseline plus the action-diversity penalty, and the full method stack with the freshness-managed buffer. The full-stack \textit{test}$_{\text{beam}}$ curve is reconstructed by re-evaluating saved checkpoints; it is noisy because per-checkpoint accuracy fluctuates, which motivates checkpoint selection by early-stopping.}
...
\bottomrule
\end{tabular}
\caption{\textbf{Per-class test accuracy} on \texttt{mixed\_v2\_easy} (value-guided beam). The baseline (\textsc{ppo-tree-rc-buf-cov}, no anti-loop / cube-root / fresh-buffer) solves only the quadratic class; the full stack solves all four change-of-variables classes at $87$--$100\%$.}
\label{tab:per_class}
\end{table}
...
\textbf{Analytic, not learned, change-of-variables.} The CoV macroaction calls an analytic routine that pattern-matches the four template families and returns the exact substitution. The agent learns \emph{when} to invoke a CoV and how to compose it, but not the substitution itself. A fully generative $\pi_{\text{cov}}$ -- proposing substitutions for equations outside the four families -- remains future work; our earlier attempts to train it directly were too brittle to include here.
