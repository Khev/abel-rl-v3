...
\textbf{Per-class accuracy.} Table~\ref{tab:per_class} breaks the result down by source class. The full stack solves all four change-of-variables classes (quadratic, cubic, quartic, exponential) at $87$--$100\%$, whereas the baseline solves only the quadratic class. The exponential class, which was $0\%$ for every checkpoint without the full stack, reaches $87\%$. The single remaining weak spot is the depth-3 closed class \texttt{abel\_level3} ($35\%$), whose equations require the longest, least templated solution paths -- a search-horizon problem rather than a missing-action one.
...
\textbf{Single-seed headline.} The $0.80$ result is one seed (\texttt{seed14000}). We report a 3-seed sweep at the (weaker) flat-buffer configuration (range $0.41$--$0.62$) and a controlled buffer-variant comparison, but a 5-seed sweep at the full fresh-buffer configuration is still in progress. The claim we are confident in is the \emph{ordering} of configurations, not a precise mean.

\textbf{Late-training drift.} Test accuracy peaks mid-training and degrades toward the end of the $3\text{M}$-step budget; the $0.80$ figure is from an early-stopped checkpoint, $0.28$ points above the final-step checkpoint. We do not yet understand the drift mechanism. Our reported numbers depend on checkpoint selection, which we make explicit.
