...
\end{align}
where the rhs of each shows a solving CoV (the polynomial families are restricted to fully-depressable special forms; see Appendix). The key design choice is \emph{how} to handle the CoV. We do not learn the substitution generatively. Instead we expose the CoV as a single \emph{macroaction} within the same dynamic action space as the closed-equation manipulations: when the policy selects the CoV macroaction, the substitution $\phi$ is computed analytically (the pattern-match is exact for the four families above) and applied to the current equation. The agent therefore learns only \emph{when} to invoke a CoV and how to compose it with ordinary algebraic steps -- not the substitution itself. This keeps the open-equation problem inside the same MDP and policy as the closed case, and a single policy is trained on a mixed dataset of closed and open equations (Section~\ref{sec:mixed}).

\subsection{Mixed dataset: closed + open equations together}
\label{sec:mixed}
We construct three mixed datasets of closed and open equations: \texttt{mixed\_v2\_tiny} (100/10), \texttt{mixed\_v2\_easy} (916/91, stratified across the four CoV classes plus \texttt{abel\_level1/2/3} closed equations), and \texttt{mixed\_v2\_large} (10K/1K), mirroring the closed-equation size convention. A single TreeMLP-PPO policy is trained on each. Initial 3-seed runs of \textsc{ppo-tree-rc-buf} on \texttt{mixed\_v2\_easy} reach \textit{coverage}$=1.0$ on the training set but only \textit{test}$_{\text{beam}}\approx 0.28$ with plain beam search, with high seed variance (one strong seed at $0.28$--$0.31$, two slower seeds at $0.06$--$0.15$). Inspecting greedy traces on the test set reveals a single dominant failure mode (Section~\ref{sec:antiloop}): the agent memorizes one ``depressed-quartic'' solution recipe and falls into repeated \texttt{relabel\_const} loops on equations that need a different finisher. The rest of this section develops the interventions that take this $0.28$ to $0.80$.
