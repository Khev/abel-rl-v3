...
def make_agent(
    ...
    if 'tree' not in agent:
        ...
        return PPO('MlpPolicy', env=env, policy_kwargs=policy_kwargs, seed=seed, verbose=0, n_steps=2048)
    ...
    # Infer limits from (possibly vectorized) env observation space
    ...
    return PPO(policy="MultiInputPolicy", env=env, policy_kwargs=kwargs, verbose=0, seed=seed, n_steps=2048)
...
def _policy_action_probs(model, obs) -> np.ndarray:
    ...
    return probs  # (n_actions,)

def _snapshot_env(env) -> Dict[str, Any]:
    return dict(
        lhs=env.lhs, rhs=env.rhs, obs=env.obs,
        current_steps=env.current_steps,
        main_eqn=env.main_eqn,
        map_constants=getattr(env, "map_constants", None),
        map_constants_history=list(getattr(env, "map_constants_history", [])),
        _timeout_count=getattr(env, "_timeout_count", 0),
    )
...
def _restore_env(env, snap: Dict[str, Any]) -> None:
    ...
    env.rhs = snap["rhs"]
    env.obs = snap["obs"]
    env.current_steps = snap["current_steps"]
    env.main_eqn = snap["main_eqn"]
    env.map_constants = snap["map_constants"]
    env.map_constants_history = list(snap["map_constants_history"])
    env._timeout_count = snap["_timeout_count"]
...
def beam_solve_one(model, env, eqn, *, beam_width=5, topk_per_node=5,
    ...
    try:
        with time_limit(per_eqn_seconds):
            ...
            # beam: (score_logprob, snapshot, obs, solved)
            ...
            for _ in range(max_steps):
                ...
                for score, snap, obs, solved in beam:
                    ...
                    probs = _policy_action_probs(model, obs)
                    if probs.ndim != 1:
...
class TrainingLogger(BaseCallback):
    def __init__(self, algo_name: str, train_env, eval_env, eval_interval: int, log_interval: int, save_dir: str, verbose=1):
        ...
        self.test_acc: List[float] = []   # greedy
        self.test_beam: List[float] = []  # beam
        self.test_at10: List[float] = []  # success@10
    ...
    def _log_eval(self, step: int):
        ...
        timed_print(f"[{self.algo_name}] t={step}: coverage={cov:.3f} | test_greedy={tst_s} | test_beam={tbeam_s} | test@10={t10_s}")
    ...
    with ProcessPoolExecutor(max_workers=n_workers, mp_context=ctx) as ex:
        ...
        for fut in as_completed(futures):
            try:
                ...
                timed_print(
                    f"✓ [{done}/{total}] {metrics['agent']} seed={metrics['seed']} "
                    f"| coverage_final={metrics['coverage_final_rate']:.2f} "
                    f"| test_greedy={metrics['final_test_acc']:.2f} "
                    f"| test_beam={metrics['final_test_beam']:.2f} "
...
if __name__ == "__main__":
    ...
    train_group.add_argument('--Ntrain', type=int, default=6*10**6, help='Total training timesteps')
    ...
    eval_group.add_argument('--eval_interval', type=int, default=10**5, help='Evaluation interval (steps)')
    eval_group.add_argument('--log_interval', type=int, default=10**5, help='Log interval (steps)')
    ...
    sys_group.add_argument('--n_workers', type=int, default=3, help='Number of parallel workers')
    ...
    for agent in agents:
        ...
        for t in range(n_trials):
            ...
            jobs.append((
                ...
                n_envs,
                # pass success replay flags through
    ...
    # Still save the full table for spreadsheets
